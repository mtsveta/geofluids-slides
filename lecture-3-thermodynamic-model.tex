...
\begin{itemize}[<+->]
\item The \alert{\textbf{activity of $i$-th gaseous species}} can be calculated using:
\[
...
\end{itemize}
\item The \textbf{fugacity coefficients of ideal gases} are $\varphi_{i}=1$. \text{But!}: the gases rarely behaving ideal.  
\end{itemize}
\end{frame}
%
% --------------------------------------------------------------------------------------------------------------------%
...
%\]
\item \textbf{Note:} For CO$_{2}$, ${T_{\mathsf{c}}=304.2\;\mathsf{K}}$, ${P_{\mathsf{c}}=73.83\;\mathsf{bar}}$,
${\omega=0.224}$. 
\end{itemize}
...
in which \textbf{\alert{liquid and vapor coexist}}.
\item There are three \textbf{isothermals}: \alert{\textbf{subcritical}},
\alert{\textbf{critical}}, and \alert{\textbf{supercritical}}. 
\item The horizontal  \textbf{blue segment} corresponds to the fluid change from liquid to vapor at a constant $T$ and $P$. 
\end{itemize}
\rcol
...
using a \alert{cubic equation of state}:
\[
Z^{3}+AZ^{2}+BZ+C=0,
\]
...
\end{itemize}
\item Any \textbf{root inside the green region} is a \textbf{mathematical artifact} of the cubic model.
\item Since we are interested in vaour (CO$_{2}$(g)) properties, we will be rather looking at the largest 
root of cubic equation. 
%\item \textbf{Question}: Which value of $Z$ are we interested
%in for CO$_{2}$(g)? $T_{\mathrm{cr}}\approx\unit[31.04]{^{\circ}C}$
\end{itemize}
\rcol
...
% --------------------------------------------------------------------------------------------------------------------%
%
\begin{frame}[fragile]{Calculating the compressibility factor of CO$_{\boldsymbol{\sf 2}}$ -- Fixed point iteration}
\begin{columns}[t]
...
$T_{\sf cr} = $  31.04 °C and pressures close to the saturation pressure (i.e.,
near the blue curve).
...
\[
\boxed{r(Z)=1+\beta-q\beta\frac{Z-\beta}{(Z+\epsilon\beta)(Z+\sigma\beta)}-Z}.
\]
...
\]
%
\item and \textbf{acceptance criterion} $|r(Z)|<\varepsilon$, where $\varepsilon$ is selected tolerance.
\end{itemize} 
\end{itemize}
\end{frame}
%
...
\begin{figure}
\centering
\includegraphics[width=1\columnwidth]{figures/activity-models/co2-compressibility-factor-newton}
...
\column{0.5\textwidth}
\begin{itemize}[<+->]
\item The \alert{Gibbs energy} is defined as
\[
...
% --------------------------------------------------------------------------------------------------------------------%
%
\begin{frame}[shrink=15]{Summary on the calculation of activities of aqueous species}
\begin{columns}[t]
...
\]
\item The \alert{activity coefficient} $\gamma_{i}$ for  \alert{CO$_{2}$(aq)}:
\[
