...
Investigation of CO$_{2}$ and calcite solubilities in the brine \\ using
the Gibbs energy minimization approach\\
...
\begin{table}
\begin{centering}
\caption{\label{tab:chemical-system-example}Considered chemical system for  solubility calculation of CO$_{2}$ and
CaCO$_{3}$ in NaCl brine.}
\par\end{centering}
\centering{}%
\begin{tabular*}{1\columnwidth}{@{\extracolsep{\fill}}ccccccccc}
\toprule 
\multicolumn{2}{c}{Species} &  & \multicolumn{2}{c}{Phases} &  & \multicolumn{3}{c}{Elements}\tabularnewline
\midrule
$\alpha_{1}$ & H$_{2}$O(l) &  & \multirow{9}{*}{$\phi{}_{1}$} & \multirow{9}{*}{Aqueous} &  & $\varepsilon{}_{1}$ & H & \emph{hydrogen}\tabularnewline
...
$\alpha_{10}$ & CO$_{2}$(g) &  & \multirow{1}{*}{$\phi{}_{2}$} & \multirow{1}{*}{Gaseous} &  &  &  & \tabularnewline
$\alpha_{11}$ & CaCO$_{3}$(s, calcite) &  & $\phi{}_{3}$ & Calcite &  &  &  & \tabularnewline
\bottomrule
\end{tabular*}
\end{table}

This project is dedicated to chemical equilibrium calculations based on a Gibbs energy minimization approach for the system shown in Table~\ref{tab:chemical-system-example}.
These simulations must be performed with Python. The script must be improved and used to study the solubility calculations of carbon dioxide (CO$_{2}$) and calcite (CaCO$_{3}$) in saline water.
%
The improvements include:
%
\begin{enumerate}
\item Implementation of more accurate models for the activities, $a_{i}$,
of aqueous and gaseous species to replace the current ones based on
ideal solution and ideal gas models. The activity models to be implemented
...
governing the chemistry of fluids or models calibrated
...
range capability of the code. The latter will permit more accurate 
equilibrium calculations, i.e., solubility of gases and minerals, at higher 
temperatures and pressures. The obtained results
will allow one to identify how increasing temperature and pressure
affect the solubility of CO$_{2}$ and CaCO$_{3}$.
\end{enumerate}
%
Once the required model improvements have been carried out in the
code, use it to perform chemical equilibrium calculations, resulting
in an equilibrium state. It contains such essential characteristics as 
the molar amounts, activities, and concentrations of the species at 
equilibrium that can be appropriately used to plot the
missing curves in the figure templates shown in Figure~\ref{fig:assigned-figures}.
Each sub-figure of Figure~\ref{fig:assigned-figures} must be generated 
by the code automating the following processes:
...
and generating plots using Python.
...
\par\end{centering}
\caption{\label{fig:assigned-figures} Figures required to be generated in the
project. }
\end{figure}

\section{Implementing activity models for aqueous and gaseous species\label{sec:implementing-activity-models}}

Provided Python code calculates the equilibrium state of the chemical
...
method}. Problem \eqref{eq:gibbs-mim-problem} consists of calculating the molar
amounts of the species ${n=(n_{1},\ldots,n_{\mathrm{N}})}$ that
minimizes the Gibbs energy of the system $G$. This minimization
...
\end{equation}
%
In other words, the amounts of the species at equilibrium must preserve the
given amounts of the elements, here denoted by ${b=(b_{1},\ldots,b_{\mathrm{E}})}$.
The second constraint requires that the amounts of the species at equilibrium
is non-negative:
%
\begin{equation}
n\geq0.
...
\end{equation}
%
Inequality \eqref{eq:nonnegativity} implies that the constraint must be satisfied for
each $i$th species, i.e.,  ${n_{i}\geq0}$, ${i=1,\ldots,\mathrm{N}}$. 

The \emph{chemical potential} of the species $\mu_{i}$ is defined as
%
\begin{equation}
...
of the $i$th species, ${R=\unit[8.31446]{J/mol}}$ is the \emph{universal
gas constant}, $T$ is \emph{temperature}, and $a_{i}$ is the \emph{activity}
of the $i$th species. 
...
The activities of the species $a_{i}$ can be thought of as their effective
...
\subsection{Activity model for aqueous solution
	\label{subsec:activity-model-aqueous}}
...
species $\gamma_{i}$ using the Davies model.}
\end{figure}

\begin{figure}[!th]
\begin{centering}
\includegraphics[width=12cm]{figures/activity-water-davies-vs-ideal}
\par\end{centering}
\caption{\label{fig:activity-water-davies}Activity of water species  $a_{\mathrm{H_{2}O(l)}}$ 
using the Davies model and the ideal solution model.}
\end{figure}
...
$-2$, respectively. Alternatively, they can be \emph{neutral}, such as CO$_{2}$(aq).
...
solvent species, H$_{2}$O(l). However, before activity models for these species
are presented some important definitions of concentration
...
\end{equation}
%
where $M_{\mathrm{H_{2}O(l)}}$ is the \emph{mass of solvent water H$_{2}$O(l)} in kg. The unit of molality is \emph{molal},
or equivalently mol/kgw, i.e., the number of moles of a solute divided
by the mass of solvent water in kg. 
%
Using the molar mass of water, ${\mathcal{M}_{\mathrm{H_{2}O}}=\unit[18.01528\cdot10^{-3}]{kg/mol}}$,
we can use the following equation instead to calculate the molality
of solutes:
\begin{equation}
m_{i}
=\frac{n_{i}}{{\mathcal{M}_{\mathrm{H_{2}O}}}\, n_{\mathrm{H_{2}O(l)}}}
=55.5084\frac{n_{i}}{n_{\mathrm{H_{2}O(l)}}}.
\end{equation}
...
of solutes), two such aqueous solutions with different composition
tend to behave similarly thermodynamically when their \emph{ionic
strength} is the same. The ionic strength of an
...
The  \emph{activity of aqueous ionic species} is defined as
\begin{equation}
a_{i}=\gamma_{i}m_{i},
\end{equation}
...
%In the provided Python code, ${\gamma_{i}=1}$,
%which assumes that the saline water or brine
%is thermodynamically an ideal solution. 
...
\end{equation}
where $A_{\gamma}$ is a Debye--H\"{u}ckel parameter defined
as:
\begin{equation}
...
are the \emph{density} and \emph{dielectric} constant of pure water. In this project,
use ${A_{\gamma}=0.5095}$, which corresponds to its value at 25°C
and 1~bar. The behavior of  $\gamma_{i}$ with respect of changing ionic strength is illustrated in Figure~\ref{fig:activity-coefficient-ions}. 

\textbf{Note:} To convert from $\log_{10}$ to $\ln$
use
\[
\ln\gamma_{i}=\ln10\,\log_{10}\gamma_{i}.
\]
...
for the activities of ionic species ({not addressed in this course}).
However, we saw in class that for the range of ionic strengths for
which the Davies model is valid (i.e., 0.1\textendash 0.7 molal) the above
equation and the simplification for an ideal solution model
%
\begin{equation}
	\ln a_{\mathrm{H_{2}O(l)}}=-\frac{1-x_{\mathrm{H_{2}O(l)}}}{x_{\mathrm{H_{2}O(l)}}}.
\end{equation}
%
results in very similar values (see Figure~\ref{fig:activity-water-davies}).
...
${c_{4}=0.4445}$ and ${c_{5}=-1.606\cdot10^{-3}}$, $T$ is the temperature
...
respectively. The dependence of the CO$_{2}$(aq) activity coefficient
 $\gamma_{\mathrm{CO_{2}(aq)}}$ on ionic strength $I$ see in Figure~\ref{fig:activity-coefficient-co2-drummond}.
...
\par\end{centering}
\caption{\label{fig:phase-diagram-pv}Pressure-volume phase diagram for a substance. Dashed lines correspond to a cubic equation of state modeling the $PVT$ behavior of the substance.}
\end{figure}
...
for different equation of state models in Table~\ref{tab:eos-params}). Here, $a$ and $b$ are
\begin{align}
...
temperature} and \emph{critical pressure} of the gas, 
$\Psi$ and $\Omega$ are given constants, and $\alpha(T_{\mathrm{r}},\omega)$ is a given
...
\end{equation}
%
respectively. Finally, $\omega$ is the \emph{acentric factor} of the substance, needed
in the evaluation of function $\alpha(T_{\mathrm{r}},\omega)$ for
some models (see Table~\ref{tab:eos-params}), with ${\omega=0.224}$
for CO$_{2}$. 

\begin{algorithm}[t]
\caption{\label{alg:comp-factor-gas-fixed-point}Algorithm for
the compressibility factor calculation using fixed-point iteration.}

\setstretch{1.5}
\begin{algorithmic}[1]
	\Function{CompressibilyFactor}{$T, P; T_\mathrm{c}, P_\mathrm{c}, \Psi, \Omega, \omega, \epsilon, \sigma$}
	...
	\State $f(Z) \gets 1+\beta-q\beta\frac{Z-\beta}{(Z+\epsilon\beta)(Z+\sigma\beta)}$
	 \Comment{Define $f(Z)$ to be used in the fixed-point algorithm.}
	\State \textsc{FixedPoint}$(Z, f(Z))$ \Comment{Call fixed-point iterative algorithm.}
...
\begin{algorithm}[t]
	\caption{\label{alg:comp-factor-gas-newton}Algorithm for
		the compressibility factor calculation using Newton iteration.}
	...
	\begin{algorithmic}[1]
		...
		\State $q \gets \frac{\Psi}{\Omega}\frac{\alpha}{T_{\mathrm{r}}}$ \Comment{Using Eq.~(\ref{eq:cubic-eos-q}).}
		\State $Z \gets 1$ \Comment{Initial guess for $Z$ corresponding to the compressibility factor of an ideal gas.}
		 \State $r(Z) \gets 1+\beta-q\beta\frac{Z-\beta}{(Z+\epsilon\beta)(Z+\sigma\beta)}-Z$
		 \Comment{Define residual function for the Newton method.}
		\State $r^{\prime}(Z) \gets -\frac{q\beta}{(Z+\epsilon\beta)(Z+\sigma\beta)}\left\{ 1-(Z-\beta)\frac{2Z+(\epsilon+\sigma)\beta}{(Z+\epsilon\beta)(Z+\sigma\beta)}\right\} -1$ 
		\Comment{Define residual function's derivative.}
		\State \textsc{Newton}$(Z, r(Z), r^{\prime}(Z))$ \Comment{Call Newton iterative algorithm.}
		\EndFunction
	\end{algorithmic}
\end{algorithm}

\begin{algorithm}[t]
	\caption{\label{alg:fixed-point}Fixed-point method.}
\setstretch{1.5}
\begin{algorithmic}[1]
	\Function{FixedPoint}{$Z, f(Z)$}
  \State $i_\textrm{max} = 100$ \Comment{Maximum number of iterations.}
  \State $ \epsilon_\textrm{tol} = 1e-4$ \Comment{Error tolerance.}
  \For{$i \gets 1 \textrm{ to } i_\textrm{max}$}
     \State $Z_\textrm{new} \gets f(Z)$ \Comment{Using Eq.~(\ref{eq:cubic-eos-reduced-vapor-root}).}
	 \If{$|Z_\textrm{new}-Z|/Z < \epsilon_\textrm{tol}$} \Comment{We stop when $Z_\textrm{new}$ is relatively close enough to $Z$.}
       \State \Return{$Z_\textrm{new}$}
	 \EndIf
     \State $Z \gets Z_\textrm{new}$
...
\begin{algorithm}[t]
\caption{\label{alg:newton}Newton's method.}

\setstretch{1.5}
\begin{algorithmic}[1]

\Function{Newton}{$Z, r(Z), r'(Z)$}
  \State $i_\textrm{max} = 100$ \Comment{Maximum number of iterations.}
  \State $ \epsilon_\textrm{tol} = 1e-4$ \Comment{Error tolerance.}
  \For{$i \gets 1 \textrm{ to } i_\textrm{max}$}
     \State $Z_\textrm{new} \gets Z - r_Z/r^{\prime}_{Z}$ 
	 \If{$r_Z < \epsilon_\textrm{tol}$} \Comment{We stop when residual $r(Z)$ is sufficiently close to zero.}
       \State \Return{$Z_\textrm{new}$}
	 \EndIf
     \State $Z \gets Z_\textrm{new}$
  \EndFor
...
and pressure $P_{\mathrm{c}}$. When the temperature and
pressure of the substance is greater than their respective critical
values, the substance is said to be in a \emph{supercritical
state}. This is a physical state in which the substance behaves similarly
to both liquid and gas (e.g., the substance is dense, like a liquid,
...
\]
it follows that there exists at most three values (i.e., three roots)
for $Z$ that satisfies \eqref{eq:cubic-equation}. When these
three roots are real numbers, the smallest value corresponds
to a liquid state, and the largest value corresponds to a gas state. 
There also could be only one real root and a conjugate pair of complex roots! The middle value is not physically relevant (see Figure~\ref{fig:phase-diagram-pv}).
...
\begin{equation}
Z=f(Z) = 1+\beta-q\beta\frac{Z-\beta}{(Z+\epsilon\beta)(Z+\sigma\beta)}.\label{eq:cubic-eos-reduced-vapor-root}
\end{equation}
...
r(Z)=f(Z) - Z = 0,
\end{equation}
where $r$ is the \emph{residual function} defined as
\begin{equation}
r(Z)=1+\beta-q\beta\frac{Z-\beta}{(Z+\epsilon\beta)(Z+\sigma\beta)}-Z.
\end{equation}
Newton's method requires the first-order derivative $r^{\prime}(Z)$
\begin{equation}
...
iterative steps:
\begin{align}
Z^{1} & =Z^{0}-r(Z^{0})/r^{\prime}(Z^{0}) & \qquad & 1^{\text{st}}\text{ iteration}\nonumber \\
 & \hphantom{=1+\beta-q\beta}\vdots &  & \qquad\vdots\\
Z^{i+1} & =Z^{i}-r(Z^{i})/r^{\prime}(Z^{i}) &  & i^{\text{th}}\text{ iteration}\nonumber 
\end{align}
until iteration $i$ satisfies
\begin{equation}
r(Z^{i+1})<\epsilon_{\mathrm{tol}},
\end{equation}
where $\epsilon_{\mathrm{tol}}$ is a tolerance for the residual function $r$. See Algorithm~\ref{alg:newton}
...
\toprule 
\multirow{2}{*}{} & \multirow{2}{*}{$T$ {[}Â°C{]}} & \multicolumn{3}{c}{$P$ {[}bar{]}}\tabularnewline
...
CO$_{2}$ and CaCO$_{3}$ at 25~°C and 1~bar only, since
the code only uses \emph{standard chemical potentials of the species}
$\mu_{i}^{\circ}$ at these temperature and pressure values. 
...
To perform  needed calculations at a given $T$ and $P$, , $\mu_{i}^{\circ}$ needs to be interpolated from the values in the table. In what follows, we describe
...
the discretization of the range $[x_{L},x_{R}]$, and $y_{j}$ corresponds to the $y$-coordinate of the $j$th point in the the discretization of the
...
The latter can be used to calculate $\mu_{i}^{\circ}(T,P)$
...
to the temperature and pressure, whereas the function $f$ stands for the standard chemical potential of the species $\mu_{i}^{\circ}$.
...
of function $f$ and its interpolation using these values. \emph{(a)}~The grid where the values
...
your machine or using some website that offers a development environment for Python, such as \texttt{repl.it} at \\
%
\begin{minipage}[t]{1\columnwidth}%
\begin{center}
	\vskip 3pt
\texttt{\href{https://repl.it/languages/python}{https://repl.it/languages/python}.}\\[5pt]
\vskip 5pt
\par\end{center}%
\end{minipage}

If you prefer to run it on your local machine, you will need to
install Python 3, if it is not already natively provided by your
operating system (e.g., by Linux and MacOS). For Windows users,
you can find the Python installer at \\
%
\begin{minipage}[t]{1\columnwidth}%
\begin{center}
\vskip 3pt
\texttt{\href{https://www.python.org/downloads/release/python-2713/}{https://www.python.org/downloads/release/python-2713/}}.\\[5pt]
\vskip 5pt
\par\end{center}%
\end{minipage}

Once Python is available in your machine, you can execute the file
\texttt{project.py} in a terminal using \texttt{}~\\
\texttt{}%
\begin{minipage}[t]{1\columnwidth}%
\begin{center}
	\vskip 3pt
\texttt{\$ python project.py}\\[5pt]
\vskip 5pt
\par\end{center}%
\end{minipage}\\
...
\texttt{cd} to change to the correct directory. Alternatively, you can
install the SublimeText editor, open \texttt{project.py}, and press
$\mathrm{Ctrl+b}$. 

Whichever way you will execute the source-code, 
a \texttt{results.txt} file should be output in the
same directory. Its content should be similar to what
is shown in Figure~\ref{fig:output-results}. The output file shows
...
In this function, \texttt{T} is the temperature in K, \texttt{P} is the pressure
in Pa, and \texttt{b} is the array with the molar amounts of the elements.
The returned object, \texttt{equilibriumstate}, contains 
...
\begin{lstlisting}[language=Python, caption=${\sf component\_ amounts()}$ function]
	def component_amounts(kgH2O, molCO2, molNaCl, molCaCO3):
	...
	return b
\end{lstlisting}
%
which can be used in the example to calculate the pH of the aqueous solution:
%
...
One of the figures to be plotted as part of this project is a pH versus
amount of CO$_{2}$ (see Plot4 in Figure~\ref{fig:assigned-figures}). Here, a series of equilibrium calculations need to be performed
...
quantities that will serve for its immediate use in a plot or for calculating another quantity of interest (e.g., pH).
...
%potentials corrected for temperatures and pressures other than 25~Â°C
%and 1~bar.
%\end{itemize}
