...
\begin{frame}{Stiffness of system of ODEs}
	\begin{itemize}
		\item \alert{\bf Stiffness}: how hard is to solve a system of equations. 
		\item \alert{\bf Why?} The system of coupled equations for combustion has very different time scale
		characteristics. In geochemistry, the situation is similar, but a little less critical.
		\item If {\bf nonstiff methods} are employed to solve stiff problems $\Rightarrow$ more computational is required.
		One example is the propagation of flame fronts
	\end{itemize}
\end{frame}

\begin{frame}{Mathematical definition of stiffness}
\begin{itemize}
	\item 
%	Stiff equations and systems:
% Let  > 0: small parameter. Consider the initial value problem
%8<
%:
%dx(t)
%dt
...
%
% --------------------------------------------------------------------------------------------------------------------%
% Numerical schemes of integration of systems of ODEs
% --------------------------------------------------------------------------------------------------------------------%
%
\begin{frame}{Numerical schemes of integration of stiff systems of ODEs}
...
\begin{frame}{Example, Roberts system}
	\vskip 10pt
	\begin{itemize}
		\item Consider the Roberts system modeling  \alert{\bf 3-species chemical kinetics problem}: 
		find amount $n = [n_{\mathsf{A}}, n_{\mathsf{B}}, n_{\mathsf{C}}]^{\mathrm{T}}$ 
		on the time-interval $t \in [0, 4 \cdot 10^{3}]$ with
		%
		\begin{itemize}
			\item reaction rates $a = 4 \cdot10^{-2}$, $b = 10^{4}$, $c = 3\cdot10^{7}$ and 
		...
		\begin{columns}[t]
			...
			\column{.6\textwidth}
			\begin{figure}[!h]
				\includegraphics[width=0.85\textwidth]{figures/chemical-kinetics/approximations}
