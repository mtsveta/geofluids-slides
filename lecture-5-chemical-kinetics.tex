...
\begin{figure}
	\vskip 5pt
	\centering
	\includegraphics[height=0.4\columnwidth]{figures/chemical-kinetics/lotka-volterra-dynamics.png}
...
\begin{frame}[<+->][shrink]{Global and elementary chemical kinetics reaction}
	...
	\begin{itemize}
		...
		\begin{itemize}
			\item {\it unimolecular reaction}: dissociation/isomerization of one reagent molecule;
		...
		%\underbrace{State A}_{Activation energy A} $\rightarrow$ \underbrace{State B}_{Activation energy B}
		\begin{itemize}
...
\begin{itemize}
	...
	\item {\bf Example}: 
	overall reaction equation for the \alert{combustion (oxidation) of hydrogen}: %/ water production:
	%
	...
	%
	\item Chemical systems described by a single chemical reaction (reactands $\rightarrow$ products) are {\bf very rare}. 
	\item In reality, 
	%
	\begin{multline*}
		\mbox{reactants} 
...
%		\begin{itemize}
%			\footnotesize
%			\item hydrocarbon + OH $\rightarrow$ alkyl radical + H$_2$O, 
%			\item alkyl radical + O$_2$ ($^3\sum$) $\rightarrow$ alkylperoxy radical, 
%			\item alkylperoxy radical + NO $\rightarrow$ alkoxy radical + NO$_2$, 
%			\item alkoxy radical + O$_2$($^3\sum$) $\rightarrow$ aldehyde + HO. 
%		\end{itemize}
		%
...
% --------------------------------------------------------------------------------------------------------------------%
%
\begin{frame}[<+->][shrink]{Reaction rate via concantrations of reactants}
%
\begin{itemize}
...
% --------------------------------------------------------------------------------------------------------------------%
%
\begin{frame}[<+->][shrink]{Stoichometry in intermediate reactions}
	%
	\begin{itemize}
		\item 
	The overall reaction of hydrogen combustion $\mathsf{2 \, H_2 + O_2 \rightleftharpoons 2\, H_2O}$ should contain the following intermediate steps (amoung total 30-40 steps):
	{\small
		\lcol
		\begin{alignat*}{2}
	...
	\item Each {\bf elementary reaction step $i$} can be characterised by the stoichiometric equation
	%
	\[
...
\begin{frame}{Example: kinetics law of mass action for hydrogen combustion}
		...
		decomposed into the following $M=6$ ementary steps with corresponding 
		the stoichometry matrix $\nu \in \mathbb{R}^{6 \times 8}$
			\lcol
			{\scriptsize
			\begin{alignat*}{4}
...
\begin{frame}[<+->][shrink]{Reaction mechanisms}
	...
	\begin{itemize}
		...
		%
		%
		\item {\bf In parallel reactions} (competitive reactions), the reagent is consumed by two different reaction paths to form 
		different products:
		%
...
\begin{frame}{Example, Complex souring system}
	...
	\begin{itemize}
		...
		{\scriptsize
		\begin{itemize}
\item Calcite,      $k_{\mathsf{Calcite}} \sim 10^{-1}$,    \qquad \qquad \quad Siderite,  $k_{\mathsf{Siderite}}  \sim 10^{-6}$,
...
\begin{frame}[<+->][shrink]{Quasi-steady-state approximation}
	...
	\begin{itemize}
		...
		%		
		\item For the mechanism	$\mathsf{A} \xrightarrow[]{k_1} \mathsf{B} \xrightarrow[]{k_3}  \mathsf{C}$, the production rates are
		%
		\begin{alignat*}{2}
...
\begin{frame}[<+->]{Evolutionary system of ODE for kinetically controlled species}
	...
	\begin{itemize}
		...
		\item $f_{i} :\mathbb{R}^{2+N}\rightarrow\mathbb{R}$ isthe \emph{production/consumption} of the $i$th species in every reaction, and 
%		\[
...
\begin{frame}{Mathematical characteristics of kinetic system of ODEs}
	\begin{itemize}
		...
		where
		\begin{itemize}
			\item $n \in \mathbb{R}^{N}$ is a vector of species concetrations,
...
%
% --------------------------------------------------------------------------------------------------------------------%
% Example, Barite precipitation as result of mixing
% --------------------------------------------------------------------------------------------------------------------%
%
\begin{frame}[fragile]{Example, Barite precipitation as result of mixing}
	\begin{itemize}
		\item Including only \alert{\bf acidic mechanism:} 
	\end{itemize}
...
% --------------------------------------------------------------------------------------------------------------------%
%
\begin{frame}[fragile]{Example, Barite precipitation as result of mixing \, ii}
	%
	\begin{itemize}
...
\begin{itemize}
	...
	\[\mathsf{Fe_{2}O_{3}(hematite)+6H^{+}=3H_{2}O(l)+2Fe^{3+}}\]
