...
\lcol
{
	...
	\begin{itemize}
		...
		%
		\pause
		\item For more general reaction\\[-5pt]
		%
...
\begin{frame}{Example, Production/consumption rate}
...
\begin{itemize}
	...
	\item $\alpha_i > 0$ is the order of reaction with respect to species $\mathsf{A_i}$, %such that $\alpha := \sum_{i}^{N} \alpha_i$ is the overall order of reaction, 
	and 
	\item $N$ is the number of species.
\end{itemize}
%
...
\begin{frame}[<+->]{Example, Differential \& integral forms for the reaction rate, Half-time}
	%
	\begin{itemize}
		\item \alert{\bf Quizz}: Given a first-order reactant and rate constant  $k$ = 1.5e-4 1/min, what is the the half-life of the reactant if the time is defined by  $t_{1/2} = \tfrac{\ln 2}{k}$?
		%
		\begin{center}
			\href{http://etc.ch/xREb}{\textcolor{indigo(dye)}{\tt http://etc.ch/xREb}} or 
...
\begin{frame}{Example, Kinetics law of mass action for hydrogen combustion}
		...
		the stoichiometry matrix $\nu \in \mathbb{R}^{6 \times 8}$
			...
			\kbordermatrix{
				...
				R_3 &   1 & -1 & 0 & 1 & 0 & 1 & -1 & 0\\[5pt]
				R_4 &  1 & -1 & 0 & 0 & -1 & 1 & 1 & 0\\[5pt]
				R_5 & -1 &  0 & 1 & 0 & 0 & 0 & 0 & 0\\[5pt]
...
\begin{frame}{Temperature dependence of rate coefficients, Arrhenius equation}
	\begin{itemize}
		...
		%
		\pause
		\item $E$ is is the {\bf minimum energy} that the reactant molecules must possess before the reaction can occur.
		\pause
		\item $E$ is {\bf determined experimentally} by performing the reaction at several temperatures. 
		\pause
...
\begin{frame}{Combustion reactions}
	...
	%
	\begin{alignat*}{2}
		& \mathsf{2Ti(s)+N2(g) \rightarrow 2TiN(s),} \\
		& \mathsf{3Mg(s)+N2(g) \rightarrow Mg3N2(s).} 
	\end{alignat*}
	%
	{\textbf{Example 2}}: chemicals can be also oxidized by other chemicals than oxygen, such as $\mathsf{H_2}$ or $\mathsf{F_2}$; these processes are also considered combustion reactions.
	\end{itemize}
	%
\end{frame}
%
...
\begin{frame}{Pre-equilibrium / partial equilibrium / fast equilibrium approximation \,ii}
	...
	\begin{itemize}
	   	\item[(ii)] If the {\bf  rates of the equilibrium reactions are much higher than the rates of the other reactions consuming 
			...
			%
			$\Rightarrow$  assuming $\mathsf{[B]} = K_e \, \mathsf{[A]}$, therefore $\tfrac{d\mathsf{[C]}}{dt} = k_3 \, \mathsf{[B]} = k_3\, K_e \, \mathsf{[A]}$.		\\[10pt]
		%
		\item \alert{\bf Quiz}: how can we express the production rate of C, if 
		$\tfrac{d\mathsf{[C]}}{dt} = k_3 \, \mathsf{[B]}$?		
		%
		\begin{center}
			\href{http://etc.ch/xREb}{\textcolor{indigo(dye)}{\tt http://etc.ch/xREb}} \quad or \quad 
			\includegraphics[height=0.18\columnwidth]{figures/chemical-kinetics/polls.png}
