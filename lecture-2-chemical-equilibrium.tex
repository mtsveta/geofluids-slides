...
or 
\quad
\includegraphics[height=0.25\columnwidth]{figures/chemical-equilibrium/poll-mass-balance.png}
\end{center}
...
\alert{\textbf{Exercise:}} Which of the following linearly dependency relation
is true for the rows of $A$:

\vskip 5pt
\visible<2->{
\centering
\href{http://etc.ch/UbWW}{\textcolor{indigo(dye)}{\tt  http://etc.ch/oyhr}} or \\
...
\textbf{To correct the correctness of the gaps in the code}, use the  \href{https://polybox.ethz.ch/index.php/s/sVotdOJMkp740Lw}{\textcolor{indigo(dye)}{\it source code}}.
...
$\mu_{i}$? 
\hiddenpause
\item \textbf{Answer:} $G$ has units of J, and $\mu_{i}$ units of J/mol.
\end{itemize}
\end{frame}
%
% --------------------------------------------------------------------------------------------------------------------%
...
what is the tendency for CO$_{2}$(g) in each of cases:
%
...
\onslide<4>{
\vskip -70pt
\includegraphics[height=0.2\columnwidth,right]{figures/chemical-equilibrium/poll-linear-independence.png}
}
\end{frame}
%
% --------------------------------------------------------------------------------------------------------------------%
...
\item \alert{\bf Warning:} Note the coefficients in the reactions and their use
...
\alert{\textbf{Exercise: }} Consider the following reaction and the subsequent
three cases:{
\[
...
\begin{itemize}
\item The \alert{\textbf{chemical potential $\mu_{i}$}} of a species is defined as:
\[
...
%
\vskip 10pt
\item \alert{\textbf{Exercise:}} At 100 °C and 50 bar, ${\mu_{\mathsf{H_{2}O\text{(aq)}}}^{\circ}=\unit[-242.992]{\mathsf{kJ/mol}}}$,
...
\centering
\vskip -15pt
\includegraphics[height=0.15\columnwidth]{figures/chemical-equilibrium/poll-lnK-log10.png}
...
video 
\href{https://polybox.ethz.ch/index.php/s/qStBnxUnry648U5}{\textcolor{indigo(dye)}{\it Mass balance and mass action equations}} with comments on this tutorial.
\end{itemize}
\end{frame}
%
%\section{Important chemical properties}
%
% --------------------------------------------------------------------------------------------------------------------%
...
\vskip -5pt
%
\item A limit of how much salt can be dissolved in a given volume of water is given by the \alert{\bf solubility product, $K_{sp}$} defined by 
%
\[
K_{sp} = [\mathsf{Ca^{2+}}]^1 \cdot [\mathsf{HCO^-_3}]^1 \cdot [\mathsf{OH^{-}}]^1,
\] 
where $[\cdot]$ denotes concentration of the species. {\bf Note}: $K_{sp}$ is \textbf{independent} of solids and liquids.

\item For calcite, $K_e \approx 10^{-12}$. 
%
...
and we assume that concentration of each species increased by equal amount 
%
\[
[\mathsf{HCO^{-}_3}] \approx (10^{-12})^{1/3} \mbox{mol/L} = 10^{-4} \mbox{mol/L}.
\] 
%
...
\begin{itemize}
\item Solubility of $\mathsf{CaCO_3}$ \textbf{increases} as the temperature of the water \textbf{decreases} ({\bf unusual}!).
...
\item \textbf{In rainwater} saturated with carbon dioxide (which decreases pH, making it more acidic), its \textbf{solubility increases} due to the formation of more soluble calcium bicarbonate 
$\Rightarrow$
%
...
\begin{frame}{Solubility of chlorargyrite, Example}
	...
	\begin{itemize}
		...
		\]
		%
		\item  \alert{\bf Question}: What is solubility of AgCl(s), where $K_{sp} = 1.8 \cdot 10^{-10}$?
		\begin{center}
			\href{http://etc.ch/oyhr}{\textcolor{indigo(dye)}{\tt http://etc.ch/oyhr}} 
			\quad
			or 
		...
		\end{center}
		\hiddenpause
		\item {\bf Answer}: 1 liter of water can dissolve $1.34 \cdot 10^{-5}$ moles of AgCl(s) at room temperature. 
		%
		\item 
		Compared with other types of salts, AgCl is poorly soluble in water.  In contrast, table salt (NaCl) has a higher $K_{sp}$ and is, therefore, more soluble.
	\end{itemize}
\end{frame}		
%
% --------------------------------------------------------------------------------------------------------------------%
...
%
\[
\boxed{ \mathrm{SI} = \log_{10} \frac{Q}{K_e}},
\] 
...
% --------------------------------------------------------------------------------------------------------------------%
%
\begin{frame}{Saturation index, Example}
...
%	IAP > Ksp: we predict that BaSO4BaSO4 will precipitate when the two solutions are mixed. In fact, BaSO4BaSO4 will continue to precipitate until the system reaches equilibrium, which occurs when 1.08×1e-10 is reached.
	...
	\begin{itemize}
		...
		\]
		%
		\item Solubility product of barium sulfate is $1.08 \cdot$1e-10 at 25 $\circ$C.
		%
		\pause
		\item  \alert{\bf Question}: What can be said about the water w.r.t. to barium sulfate if \\
